...
export function timingSafeEqual(a: string, b: string): boolean {
  ...
  // If lengths differ we must still run timingSafeEqual to avoid leaking
  // length information via timing, so we compare bufA against a same-length
  // copy of bufB (zero-padded or truncated) but always return false.
  if (bufA.length !== bufB.length) {
    // Run the comparison anyway to keep constant time characteristics.
...
export function generateOTP(digits = 6): string {
  ...
  const max = Math.pow(10, digits); // exclusive upper bound
