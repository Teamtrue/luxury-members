 ...
 *
 * Pure HTML email template functions for transactional emails.
 *
 ...
 *   - Footer includes: brand name, unsubscribe notice, CIN.
 *
...
import { brand as BRAND_CONFIG } from '@/lib/brand'

// ---------------------------------------------------------------------------
// Shared constants — mapped from lib/brand.ts
// ---------------------------------------------------------------------------

const BRAND = {
  name:         BRAND_CONFIG.name,
  obsidian:     BRAND_CONFIG.backgroundColor,
  gold:         BRAND_CONFIG.primaryColor,
  cream:        BRAND_CONFIG.cream,
  cin:          BRAND_CONFIG.cin,
...
export function welcomeEmail(
  ...
  const html = emailShell(`
    ...
    <p style="margin:0 0 24px;font-size:28px;font-weight:bold;color:${BRAND.gold};">
      ${tokenBalance.toLocaleString('en-IN')} ${BRAND_CONFIG.tokenName}s
    </p>
    <p style="margin:0 0 16px;">
...
Starting token balance: ${tokenBalance.toLocaleString('en-IN')} ${BRAND_CONFIG.tokenName}s
(1 token = ₹0.50)
    ...
    tokensUsed > 0
      ? `<tr>
           ...
           <td style="padding:6px 0;text-align:right;color:${BRAND.cream};">
             ${tokensUsed.toLocaleString('en-IN')} ${BRAND_CONFIG.tokenName}s
           </td>
         </tr>`
      : ''
  ...
  const html = emailShell(`
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0"
      ...
      <tr>
        ...
        <td style="padding:6px 0;text-align:right;color:${BRAND.cream};">
          +${tokensEarned.toLocaleString('en-IN')} ${BRAND_CONFIG.tokenName}s
        </td>
      </tr>
    </table>
...
${tokensUsed > 0 ? `Tokens Redeemed: ${tokensUsed.toLocaleString('en-IN')} ${BRAND_CONFIG.tokenName}s\n` : ''}Tokens Earned: +${tokensEarned.toLocaleString('en-IN')} ${BRAND_CONFIG.tokenName}s
...
export function membershipRenewalReminderEmail(
  ...
  const html = emailShell(`
    ...
    <p style="margin:0 0 24px;">
      ...
      ${BRAND_CONFIG.tokenName} rewards, and priority support.
    </p>
