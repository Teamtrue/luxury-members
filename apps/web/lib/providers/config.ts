 ...
 *
 * Encryption: when ENCRYPTION_KEY is set, config_encrypted stores an AES-256-GCM
 * envelope (see lib/providers/encryption.ts).  When the key is absent (dev mode),
 * credentials are stored and returned as plain JSONB.
 */
...
interface RawProviderConfigRow {
  ...
  /** AES-256-GCM encrypted credentials JSONB; decrypted by getActiveProvider via decryptConfig(). */
...
export async function loadProviderConfig(
  ...
  const config: ProviderConfig = {
    ...
    config: decryptConfig((data.config_encrypted as Record<string, string>) ?? {}),
