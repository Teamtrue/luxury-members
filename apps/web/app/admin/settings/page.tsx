...
// Providers whose implementations are stubs — configuring them will error at runtime.
const STUB_PROVIDERS = new Set(['stripe', 'payu', 'aws_ses', 'aws_sns']);
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '12px 18px', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {STUB_PROVIDERS.has(provider.provider_name) ? (
          <span style={{
            fontSize: 11, color: '#6B7280', fontStyle: 'italic', alignSelf: 'center',
          }}>
            Coming soon — not available in this release
          </span>
        ) : (
          <>
        <button
        ...
        {!provider.is_active && (
          <button
            ...
            style={{
              height: 30, padding: '0 12px', fontSize: 11, fontWeight: 600,
              background: activating ? 'rgba(201,169,97,0.5)' : 'var(--gold)',
...
export default function AdminSettingsPage() {
  ...
  const [siteConfigSuccess, setSiteConfigSuccess] = useState(false)
  ...
  const [brandFontFamily, setBrandFontFamily] = useState('Cormorant Garamond')
  // Feature flags
  const [featConcierge, setFeatConcierge] = useState(true)
  const [featReferrals, setFeatReferrals] = useState(true)
  const [featDisputes, setFeatDisputes] = useState(true)
  const [featGdprExport, setFeatGdprExport] = useState(true)
  const [featsSaving, setFeatsSaving] = useState(false)

  /* ── General / Financial / Operations state ── */
  const [clubName, setClubName]             = useState<string>(brand.name);
  const [commissionPct, setCommissionPct]   = useState(3);
  const [maxRedemption, setMaxRedemption]   = useState(20);
  const [supportEmail, setSupportEmail]     = useState<string>(brand.supportEmail);
  ...
  async function handleSave() {
    ...
    try {
      const kvPairs: Record<string, string> = {
        ...
        'brand.supportEmail': brandSupportEmail,
        'brand.primaryColor': brandPrimaryColor,
        'brand.logoUrl':      brandLogoUrl,
        'brand.fontFamily':   brandFontFamily,
        'feature.concierge':  String(featConcierge),
        'feature.referrals':  String(featReferrals),
        'feature.disputes':   String(featDisputes),
        'feature.gdprExport': String(featGdprExport),
      };
      const res = await fetch('/api/admin/site-config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ config: kvPairs }),
      });
    ...
    <div style={{ padding: 28, maxWidth: 800, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--mute-dk)', marginTop: 4 }}>
          {`Configure global parameters for ${brand.name}. Changes take effect immediately after saving.`}
        </p>
      </div>
      ...
      <div style={sectionStyle}>
        ...
        <div style={sectionBodyStyle}>
          ...
          <SliderRow
            ...
            step={5}
            unit="%"
            hint={`Maximum % of a booking value that can be paid using ${brand.tokenName}s.`}
          ...
          <div>
            ...
            />
            <p style={hintStyle}>{`${brand.tokenName}s expire after this many months of inactivity.`}</p>
          </div>
        </div>
      </div>
