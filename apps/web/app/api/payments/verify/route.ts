...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const { data: payment, error: paymentFetchError } = await db
      .from('payments')
      .select('id, booking_id, user_id, status, amount_paise, provider')
    ...
    if (!resolvedBookingId) {
      // Membership payment: activate the pending membership linked to this payment.
      const membershipId = (p.metadata as Record<string, unknown> | null)?.membership_id as string | undefined;

      const membershipQuery = membershipId
        ? db
            .from('memberships')
            .select('id, status, user_id, membership_plans ( id, name, slug )')
            .eq('status', 'pending')
            .eq('user_id', user.id)
            .eq('id', membershipId)
        : db
      ...
      if (membership) {
        ...
        // Queue welcome/activation notification.
        const plan = mem.membership_plans as Record<string, unknown> | null;
        await db.from('notifications').insert({
          user_id:       mem.user_id,
          channel:       'email',
          template_name: 'MEMBERSHIP_ACTIVATED',
          template_data: {
        ...
        await logAudit({
          action:      'payment.membership_activated',
          actor_type:  'system',
          target_type: 'membership',
          target_id:   mem.id as string,
          details:     { payment_id: p.id, user_id: mem.user_id },
        });
      }
    ...
    // 7. Credit earned tokens — guarded by reference_id uniqueness to prevent
    //    double-credit if verify and webhook both run concurrently.
    const tokensEarned = bk.tokens_earned as number;
    if (tokensEarned > 0) {
      // Check idempotency: skip if a token credit already exists for this booking.
      const { data: existingCredit } = await db
        .from('token_transactions')
        .select('id')
        .eq('reference_type', 'booking')
        .eq('reference_id', resolvedBookingId)
        .eq('type', 'earned')
        .maybeSingle();
    ...
    // Reset churn_score after confirmed payment — member is active again.
    await db.from('memberships')
      .update({ churn_score: null })
      .eq('user_id', user.id)
      .eq('status', 'active')
      .then(() => undefined);
