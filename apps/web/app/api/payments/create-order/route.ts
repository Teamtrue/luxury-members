...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    if (bookingId) {
      ...
      // Create a pending membership row so verify/route.ts can activate it
      // after payment succeeds. Generate a referral code for this membership.
      const refChars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
      const referralCode = Array.from({ length: 8 }, () => refChars[randomInt(refChars.length)]).join('');
      const { data: newMembership, error: membershipInsertError } = await db
        .from('memberships')
        .insert({
          user_id:       user.id,
      ...
      if (membershipInsertError) {
        console.error(
          '[POST /api/payments/create-order] Membership insert error:',
          membershipInsertError.message
        );
        // Non-fatal: payment can still proceed; membership will be activated
        // via user_id lookup in verify route.
      } else if (newMembership) {
        dbMembershipId = (newMembership as Record<string, unknown>).id as string;
      }
    }

    // Fraud scoring — synchronous rule-based check before calling gateway.
    const ip = getClientIP(request);
    const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
    const [profileResult, bookings24hResult, ipMembersResult] = await Promise.all([
      db.from('user_profiles').select('created_at').eq('id', user.id).maybeSingle(),
      db.from('bookings').select('id', { count: 'exact', head: true })
        .eq('user_id', user.id).gte('created_at', oneDayAgo),
      db.from('audit_logs').select('actor_id', { count: 'exact', head: true })
        .eq('action', 'payment.verified').eq('ip_address', ip ?? '').gte('created_at', oneDayAgo),
    ]);

    const memberCreatedAt = (profileResult.data as Record<string, unknown> | null)?.created_at as string | null;
    ...
    const fraudScore = scoreFraudRisk({
      ...
      lifetime_bookings:         0,
      bookings_last_24h:         (bookings24hResult.count as number) ?? 0,
      tokens_used_pct:           0,
      is_new_delivery_address:   false,
      same_ip_different_members: (ipMembersResult.count as number) ?? 0,
    });

    if (fraudScore.action === 'block') {
      await logAudit({
        action:     'payment.blocked_fraud',
        actor_type: 'member',
        actor_id:   user.id,
        details:    { fraud_score: fraudScore.risk_score, rules: fraudScore.triggered_rules },
        ip_address: ip,
      });
      return apiError(fraudScore.reason ?? 'Transaction blocked. Contact support.', 403);
    }

    if (fraudScore.action === 'flag') {
      // Insert into manual_review_queue — non-blocking (proceed with payment)
      await db.from('manual_review_queue').insert({
        user_id:     user.id,
        reason:      fraudScore.triggered_rules.join(', '),
        risk_score:  fraudScore.risk_score,
        metadata:    { amount_paise: amountPaise, payment_type: paymentType },
      }).then(() => undefined);
    }
