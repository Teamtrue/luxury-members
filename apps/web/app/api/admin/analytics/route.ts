...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    // -----------------------------------------------------------------------
    // 9. Revenue by month for last 12 months (from payments table, status=captured)
    // -----------------------------------------------------------------------
    const { data: paymentsMonthly, error: paymentsMonthlyError } = await db
      .from('payments')
      .select('amount_paise, created_at')
      .eq('status', 'captured')
      .gte('created_at', twelveMonthsAgo);

    if (paymentsMonthlyError) {
      console.error('[admin/analytics] paymentsMonthly error:', paymentsMonthlyError.message);
    }

    const revenueByMonth: Record<string, number> = {};
    for (const p of paymentsMonthly ?? []) {
      const month = (p.created_at as string).slice(0, 7);
      revenueByMonth[month] = (revenueByMonth[month] ?? 0) + (p.amount_paise as number);
    }

    // Build last-12-months array with zero-fill for months with no revenue
    const revenueLast12Months: { month: string; amount_paise: number }[] = [];
    for (let i = 11; i >= 0; i--) {
      const d = new Date();
      d.setDate(1);
      d.setMonth(d.getMonth() - i);
      const monthKey = d.toISOString().slice(0, 7);
      revenueLast12Months.push({ month: monthKey, amount_paise: revenueByMonth[monthKey] ?? 0 });
    }

    // -----------------------------------------------------------------------
    // 10. New signups last 30 days (user_profiles created_at)
    // -----------------------------------------------------------------------
    const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString();

    const { count: newSignups30d, error: newSignupsError } = await db
      .from('user_profiles')
      .select('id', { count: 'exact', head: true })
      .gte('created_at', thirtyDaysAgo);

    if (newSignupsError) {
      console.error('[admin/analytics] newSignups30d error:', newSignupsError.message);
    }

    // -----------------------------------------------------------------------
    // 11. Bookings in last 30 days
    // -----------------------------------------------------------------------
    const { count: bookings30d, error: bookings30dError } = await db
      .from('bookings')
      .select('id', { count: 'exact', head: true })
      .gte('created_at', thirtyDaysAgo);

    if (bookings30dError) {
      console.error('[admin/analytics] bookings30d error:', bookings30dError.message);
    }

    // -----------------------------------------------------------------------
    // 12. Churn risk distribution (from memberships.churn_score)
    // -----------------------------------------------------------------------
    const { data: churnData, error: churnDataError } = await db
      .from('memberships')
      .select('churn_score')
      .eq('status', 'active')
      .not('churn_score', 'is', null);

    if (churnDataError) {
      console.error('[admin/analytics] churnData error:', churnDataError.message);
    }

    const churnBuckets = { low: 0, medium: 0, high: 0, critical: 0 };
    for (const m of churnData ?? []) {
      const score = m.churn_score as number;
      if (score >= 0.75)      churnBuckets.critical++;
      else if (score >= 0.5)  churnBuckets.high++;
      else if (score >= 0.3)  churnBuckets.medium++;
    ...
    return apiSuccess({
      ...
      active_memberships:     totalActiveMembers,
      new_signups_30d:        newSignups30d ?? 0,
      ...
      churn_risk_distribution: churnBuckets,
    });
  } catch (err) {
