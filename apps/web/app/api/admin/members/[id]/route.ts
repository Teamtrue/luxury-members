  ...
  try {
    ...
    if (tier) {
      ...
      const { error: tierError } = await db
        .from('memberships')
        .update({ plan_id: plan.id, updated_at: nowIso })
        .eq('user_id', id);
      ...
      await db.from('user_profiles').update({
        last_admin_action:    'tier_changed',
        last_admin_action_at: nowIso,
        last_admin_action_by: session.adminUserId,
      }).eq('id', id);
    ...
    // Write admin notes to user_profiles (migration 024_admin_notes.sql added the columns)
    if (notes !== undefined) {
      ...
      if (notesError) {
        ...
        return apiError('Failed to update admin notes.', 500);
      }
