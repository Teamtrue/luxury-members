...
import { encryptConfig }              from '@/lib/providers/encryption';
  ...
  try {
    switch (action) {
      ...
      case 'update_config': {
        ...
        // Separate webhook_secret from the rest of the config, then encrypt.
        const { webhook_secret, ...credentialConfig } = config;

        const updatePayload: Record<string, unknown> = {
          config_encrypted:    encryptConfig(credentialConfig),
