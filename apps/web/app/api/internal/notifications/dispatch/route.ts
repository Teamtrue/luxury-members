...
function renderTemplate(
  ...
  switch (templateName) {
    ...
    case 'MEMBERSHIP_RENEWAL_7DAY':
    case 'MEMBERSHIP_RENEWAL_FINAL':
    ...
    case 'MEMBERSHIP_ACTIVATED': {
      const planName  = String(data.plan_name ?? 'Membership');
      const planSlug  = String(data.plan_slug ?? 'silver');
      const expiresAt = String(data.expires_at ?? '');
      return {
        subject: `Welcome to ${brand.name} — your ${planName} is now active!`,
        text: `Your ${planName} membership is active. Enjoy ${planSlug}-tier benefits at ${brand.url}. Your membership is valid until ${expiresAt ? new Date(expiresAt).toLocaleDateString('en-IN') : 'N/A'}.`,
      };
    }

    case 'TOKEN_EXPIRY_REMINDER': {
      const subject = `Your ${brand.name} ${brand.tokenName}s are expiring soon`;
      const text    = `Dear Member, you have ${data.expiring_tokens} ${brand.tokenName}s expiring on ${data.expires_at}. Use them before they expire at ${brand.url}/deals`;
      return { subject, text };
    }
    ...
    default: {
      ...
      return {
        subject: String(data.subject ?? `${brand.name} notification: ${templateName}`),
