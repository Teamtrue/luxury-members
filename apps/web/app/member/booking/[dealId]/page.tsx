    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 503 || json.error?.includes('not configured') || json.error?.includes('provider')) {
          setPaymentError(`Payment gateway is being set up. Please contact support at ${brand.supportEmail}`);
          setPaying(false);
          return;
        }
        ...
        <div>
          <div style={card}>
            ...
            <p style={{ fontSize: 13, color: 'var(--mute-dk)', marginBottom: 20 }}>
              {memberTier.charAt(0).toUpperCase() + memberTier.slice(1)} members can use up to {memberTier === 'obsidian' ? '50' : memberTier === 'platinum' ? '30' : '20'}% of the order value in {brand.tokenName}s.
            </p>
