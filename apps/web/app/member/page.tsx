...
export default function MemberDashboard() {
  ...
  useEffect(() => {
    const dismissed = localStorage.getItem('plutus_onboarding_dismissed');
    if (dismissed !== '1') setOnboardingDismissed(false);
  }, []);

  function dismissOnboarding() {
    localStorage.setItem('plutus_onboarding_dismissed', '1');
  ...
  // Show onboarding banner for new members: 0 bookings AND token balance ≤ 500 (welcome bonus only)
  const isNewMember = !loading && bookings.length === 0 && tokenBalance <= 500;
  const showOnboarding = isNewMember && !onboardingDismissed;
      ...
      {showOnboarding && (
        <div style={{
          ...
          <p style={{
            ...
            You have {tokenBalance.toLocaleString('en-IN')} PC Tokens ready to use. Here&apos;s how to get the most from your membership:
          </p>
          <ol style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <li style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontWeight: 700, color: '#b8860b', minWidth: 20 }}>1.</span>
              <span style={{ fontSize: 14, color: '#333' }}>
                Browse exclusive deals &rarr;{' '}
                <Link href="/member/deals" style={{ color: '#b8860b', fontWeight: 600, textDecoration: 'underline' }}>
                  Explore Deals
                </Link>
              </span>
            </li>
            <li style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontWeight: 700, color: '#b8860b', minWidth: 20 }}>2.</span>
              <span style={{ fontSize: 14, color: '#333' }}>
                Book and save &mdash; your first saving could be &#x20B9;5,000+
              </span>
            </li>
            <li style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontWeight: 700, color: '#b8860b', minWidth: 20 }}>3.</span>
              <span style={{ fontSize: 14, color: '#333' }}>
                Refer friends &mdash; earn &#x20B9;500 in tokens per referral
              </span>
            </li>
          </ol>
        </div>
